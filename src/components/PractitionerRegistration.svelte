<script lang="ts" >
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (id) {
      loading = true;
      const resp = await fhir.put(`/Practitioner/${id}`, { ...data, id });
      loading = false;
    } else {
      loading = true;
      const resp = await fhir.post(`/Practitioner`, data);
      if (resp.status == 201) {
      ehrId = resp.data.id;
        }
...
>
  <div class="field">
    <mb-input class="hidden"  path="resourceType" data="Practitioner" />
  </div>
  <br>
  <div class="field">
    <label for="" class="font-bold">Practitioner information</label>
  </div>
  <div class="field">
